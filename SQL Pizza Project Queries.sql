...
select avg(Total_Pizza_Ordered_that_day) as 'Avg Number of pizzas ordered per day'  from cte;

-- alternate using subquery
select avg(Total_Pizza_Ordered_that_day) as 'Avg Number of pizzas ordered per day' from 
(
	select orders.date as 'Date', sum(order_details.quantity) as 'Total_Pizza_Ordered_that_day'
	from order_details
	join orders on order_details.order_id = orders.order_id
...
join pizza_types on pizza_types.pizza_type_id = pizzas.pizza_type_id
group by pizza_types.name
order by Revenue_from_pizza desc limit 3;
